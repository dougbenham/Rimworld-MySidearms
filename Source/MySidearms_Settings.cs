...
using System.Linq;
using MySidearms.UI.Verse;
using MySidearms.Utilities;
using RimWorld;
using UnityEngine;
using Verse;
using static MySidearms.Utilities.Enums;
using static MySidearms.UI.SidearmsSpecificExtensions;
using static MySidearms.UI.ListingExtensions;
...
namespace MySidearms
{
    ...
    public class MySidearms_Settings : ModSettings
    {
        ...
        public override void ExposeData()
        {
            ...
            else
            {
                ...
                else if (Scribe.mode == LoadSaveMode.PostLoadInit)
                {
                    ...
                    {
                        Log.Warning($"MS: LimitMode collection contained defs. This will need a one-time migration resave.");
                        NeedsResaving = true;
                    }
                    ...
                    foreach (var defName in LimitModeSingle_Selection_AsStringCache) 
                    {
                        ...
                        if(def == null) 
                        {
                            //Log.Warning($"MS: LimitModeSingle_Selection contained unknown def {defName}. Removing.");
                            NeedsResaving = true;
                            continue;
                        }
                    ...
                    foreach (var defName in LimitModeSingleMelee_Selection_AsStringCache)
                    {
                        ...
                        if (def == null)
                        {
                            //Log.Warning($"MS: LimitModeSingleMelee_Selection contained unknown def {defName}. Removing.");
                            NeedsResaving = true;
                            continue;
                        }
                    ...
                    foreach (var defName in LimitModeSingleRanged_Selection_AsStringCache)
                    {
                        ...
                        if (def == null)
                        {
                            //Log.Warning($"MS: LimitModeSingleRanged_Selection contained unknown def {defName}. Removing.");
                            NeedsResaving = true;
                            continue;
                        }
