...
namespace MySidearms.Intercepts
{
        ...
        public static void CheckForTools(Toil toil) 
        {
            if (Settings.ToolAutoSwitch == true)
            {
                ...
                toil.AddPreInitAction(delegate
                {
                    ...
                    bool usingAppropriateTool = WeaponAssignment.EquipBestWeaponFromInventoryByStatModifiers(pawn, possiblyActiveStats.ToList());
        ...
        public static void AutoUndraftTickInterval(AutoUndrafter __instance, int delta, Pawn ___pawn, int ___lastNonWaitingTick)
        {
            ...
            if (tick % 100 == 0)
            {
                if (pawn != null && pawn.Map != null && pawn.CurJobDef == JobDefOf.Wait_Combat && pawn.stances != null && pawn.stances.curStance is Stance_Mobile)
                {
                    ...
                    WeaponAssignment.EquipBestWeaponFromInventoryByPreference(pawn, DroppingModeEnum.Combat);
    ...
    public static class Stance_Warmup_StanceTick_Postfix
    {
        ...
        public static void StanceTick(Stance_Warmup __instance)
        {
            ...
            bool swapped = WeaponAssignment.TrySwapToMoreAccurateRangedWeapon(pawn, target, MiscUtils.shouldDrop(pawn, DroppingModeEnum.Combat, false), skipManualUse, skipDangerous, skipEMP);
            if(swapped)
            {
    ...
    public static class JobDriver_AttackMelee_TryMakePreToilReservations
    {
        public static void Postfix(JobDriver_AttackMelee __instance)
        {
            ...
            {
                WeaponAssignment.ChooseOptimalMeleeForAttack(caster, target as Pawn);
            }
        }
    }
