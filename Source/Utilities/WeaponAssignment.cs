...
namespace MySidearms.Utilities
{
    public static class WeaponAssignment
    {
        public static bool EquipSpecificWeaponFromInventoryAsOffhand(Pawn pawn, ThingWithComps weapon, bool dropCurrent, bool intentionalDrop)
        {
            ...
            {
                Log.Error("MS: EquipSpecificWeaponFromInventoryAsOffhand called, but Tacticowl is not active!");
                return false;
            }
        ...
        public static void UnequipOffhand(Pawn pawn, ThingWithComps offhand, bool dropCurrent, bool intentionalDrop)
        {
            if (!(Tacticowl.active && Tacticowl.dualWieldActive()))
            {
                Log.Error("MS: UnequipOffhand called, but Tacticowl is not active!");
                return;
            }
            ...
            else
            {
                ...
                if (!addedToInventory)
                    Log.Warning(String.Format("MS: Failed to place offhand equipment {0} into inventory on pawn {1} (colonist: {2}) (dropping: {3}, current drop mode: {4}). Aborting swap. Please report this!",
        ...
        public static bool EquipSpecificWeapon(Pawn pawn, ThingWithComps weapon, bool dropCurrent, bool intentionalDrop)
        {
            ...
            {
                Log.Warning("MS: Attempted to equip already-equipped weapon!");
                return false;
            }
            ...
            {
                Log.Warning($"MS: Blocked equip of {weapon.Label} at equip-time because of: {reason}");
                return false;
            }
            ...
            if(currentPrimary != null)
            {
                ...
                else
                {
                    ...
                    if (!addedToInventory)
                    {
                        Log.Warning(String.Format("MS: Failed to place primary equipment {0} (initially was {1}) into inventory when swapping to {2} on pawn {3} (colonist: {4}) (dropping: {5}, current drop mode: {6}). Aborting swap. Please report this!",
